...
int net__try_connect(struct mosquitto *mosq, const char *host, uint16_t port, mosq_sock_t *sock, const char *bind_address, bool blocking)
{
	...
	*sock = INVALID_SOCKET;
	...
	if ((fds = _mosquitto_calloc(sizeof(struct pollfd), addrCnt)) == NULL) {
		perror("_mosquitto_calloc failed");
		return MOSQ_ERR_NOMEM;
	}
...
+				COMPAT_CLOSE(s);
		...
		if (rc == 0) {
			_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "connect successful, fd = %d", s);
			foundFd = s;
			goto end;
		} else if (errno == EINPROGRESS) {
			// store pending connection attempt
			_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "connect pending, fd = %d", s);
			...
			_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "connect failed, fd = %d, errno %d", s, errno);
		}
	}
	...
	while(foundFd == -1 && remainingConnectionCnt) {
		...
		_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "about to poll, %d fd remaining, pollWaitMs = %d", remainingConnectionCnt, pollWaitMs);
		pollResult = poll(fds, numberOfConnections, pollWaitMs);
		if (!pollResult) {
			_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "poll timed out");
			break;
			...
			_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "unexpected poll failure, errno %d", errno);
			break;
		}
		...
		for (i = 0; i < numberOfConnections; i++) {
			if (fds[i].revents && foundFd == -1) {
				...
				_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "fd %d revents 0x%04x", fds[i].fd, fds[i].revents);
				if (getsockopt(fds[i].fd, SOL_SOCKET, SO_ERROR, &error, &len) == -1) {
					// this should never happen on a valid socket
					_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "getsockopt failed for fd %d", fds[i].fd);
					...
					if (error) {
						...
						_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "received error %d on fd %d", error, fds[i].fd);
						if (error == EINPROGRESS) {
							continue;
						}
						_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "closing fd %d", fds[i].fd);
						COMPAT_CLOSE(fds[i].fd);
						fds[i].fd = -1;
						fds[i].events = 0x0;
						fds[i].revents = 0x0;
						...
						// got a connection!
						_mosquitto_log_printf(mosq, MOSQ_LOG_DEBUG, "established connection on fd %d", fds[i].fd);
						foundFd = fds[i].fd;
						break;
					}
				}
	...
	// return result
	if (foundFd == -1) {
		_mosquitto_log_printf(mosq, MOSQ_LOG_ERR, "Failed to establish connection");
