...
int net__try_connect_tcp(struct mosquitto *mosq, const char *host, uint16_t port, mosq_sock_t *sock, const char *bind_address, bool blocking, bool disable_tls)
{
	int rc;
	struct addrinfo *ainfo;
...
int net__try_connect(struct mosquitto *mosq, const char *host, uint16_t port, mosq_sock_t *sock, const char *bind_address, bool blocking, bool disable_tls)
{
	if(port == 0){
		...
		return net__try_connect_tcp(mosq, host, port, sock, bind_address, blocking, disable_tls);
	}
}
