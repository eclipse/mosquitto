		...
		for(i=0; i<mosq->bridge->topic_count; i++){
			...
			if((cur_topic->direction == bd_both || cur_topic->direction == bd_out)
				...
				if(match){
					...
					log__printf(NULL, MOSQ_LOG_DEBUG, "Sending PUBLISH to %s (d%d, q%d, r%d, m%d, '%s', ... (%ld bytes))", !(mosq->id)?"(null)":mosq->id, dup, qos, retain, mid, mapped_topic, (long)payloadlen);
...
#endif
	log__printf(NULL, MOSQ_LOG_DEBUG, "Sending PUBLISH to %s (d%d, q%d, r%d, m%d, '%s', ... (%ld bytes))", !(mosq->id)?"(null)":mosq->id, dup, qos, retain, mid, topic, (long)payloadlen);
	G_PUB_BYTES_SENT_INC(payloadlen);
#else
	log__printf(mosq, MOSQ_LOG_DEBUG, "Client %s sending PUBLISH (d%d, q%d, r%d, m%d, '%s', ... (%ld bytes))", !(mosq->id)?"(null)":mosq->id, dup, qos, retain, mid, topic, (long)payloadlen);
#endif
...
#ifdef WITH_BROKER
		log__printf(NULL, MOSQ_LOG_NOTICE, "Dropping too large outgoing PUBLISH for %s (%d bytes)", !(mosq->id)?"(null)":mosq->id, packetlen);
#else
