...
try:
    ...
    post_data = {'post_escaped_chars': 'abc\'\"%1', 'key3': 'value3'}
    r = requests.post("http://localhost:8080/get_vars", json=post_data)
    r.raise_for_status()
    rc = r.text != '{"post_escaped_chars": "abc\'\\"%1", "key3": "value3"}'
    if rc:
