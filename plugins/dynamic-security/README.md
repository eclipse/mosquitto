...
The plugin stores the defined clients, roles, groups and associated ACLs in a plain textfile. 
The format of the file is either JSON, or YAML which is preferred if the file is intended to be
viewed or changed by a human.

If the configuration file does not exist, the plugin will generate an initialize a configuration file, 
create an admin and a demo account, and set up roles, groups and ACLs accordingly. 
All passwords are auto-generated by default and stored in a file with a similar name than the 
configuration file, with a `.pw` suffix.
