...
int mosquitto_main_loop(struct mosquitto_db *db, mosq_sock_t *listensock, int listensock_count, int listener_max)
{
...
#ifdef WITH_EPOLL
	db->epollfd = 0;
	if ((db->epollfd = epoll_create(MAX_EVENTS+1)) == -1) {
	...
	while(run){
						...
						if(context->current_out_packet || context->state == mosq_cs_connect_pending){
							if(context->events != EPOLLOUT) {
								...
								if(epoll_ctl(db->epollfd, EPOLL_CTL_MOD, context->sock, &ev) == -1) {
										log__printf(NULL, MOSQ_LOG_ERR, "Error in epoll re-registering to EPOLLOUT: %s", strerror(errno));
								}
							...
							if(context->events == EPOLLOUT) {
								...
								ev.events = EPOLLIN;
								if(epoll_ctl(db->epollfd, EPOLL_CTL_MOD, context->sock, &ev) == -1) {
										log__printf(NULL, MOSQ_LOG_ERR, "Error in epoll re-registering to EPOLLIN: %s", strerror(errno));
								}
...
#ifdef WITH_EPOLL
		fdcount = epoll_wait(db->epollfd, events, MAX_EVENTS+1, 100);
#else
		...
		case -1:
			...
			break;
		case 0:
			log__printf(NULL, MOSQ_LOG_DEBUG, "Timeout in epoll waiting: %s.", strerror(errno));
			break;
		default:
			for(i=0; i<fdcount; i++){
