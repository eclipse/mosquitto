			...
			if(context->state != mosq_cs_disconnecting && context->state != mosq_cs_disconnect_with_will){
				switch(reason){
					...
					case MOSQ_ERR_ADMINISTRATIVE_ACTION:
						...
						break;
					case MOSQ_ERR_ERRNO:
						log__printf(NULL, MOSQ_LOG_NOTICE, "Client %s disconnected due to errno %d, %s.", id, errno, strerror(errno));
						break;
					default:
						log__printf(NULL, MOSQ_LOG_NOTICE, "Bad socket read/write on client %s, reason %d, disconnecting.", id, reason);
						break;
				}
			}else{
